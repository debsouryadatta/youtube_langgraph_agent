...
def create_video_with_overlays(state):
    ...
    if not state.get("avatar_video_path"):
        raise ValueError("avatar_video_path is required in state")
    ...
    try:
        original_video = VideoFileClip(state["avatar_video_path"])
        video_duration = original_video.duration
        original_width, original_height = original_video.size
    except Exception as e:
        raise ValueError(f"Error loading video from {state['avatar_video_path']}: {str(e)}")
    ...
    # Add background music path to the state
