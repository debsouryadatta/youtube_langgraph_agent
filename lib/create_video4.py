...
def create_video_with_overlays(state):
    ...
    final_audio = original_video.audio
    ...
    if "bg_music_path" in state and state["bg_music_path"] and os.path.exists(state["bg_music_path"]):
        try:
            ...
            final_audio = CompositeAudioClip([original_video.audio, bg_music])
    ...
    # Set the final audio (original + background music)
